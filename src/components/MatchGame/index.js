...
class MatchGame extends Component {
  ...
  onClickImage = id => {
    const {thumbnailItem, score, timer, isGameRunning} = this.state
    if (timer !== 0 && isGameRunning) {
      if (thumbnailItem.id === id) {
        this.setState({
          score: score + 1,
          thumbnailItem:
  ...
  render() {
    const {thumbnailItem, activeTab, timer, isGameRunning} = this.state
    const {imageUrl} = thumbnailItem
    const filteredImageList = imagesList.filter(
      eachImage => eachImage.category === activeTab,
    )
    return (
      <div className="matchgame-container">
        <GameNavBar scoreDetails={this.state} />
        <div className="card">
              ...
              <ul className="tabs-container">
                {tabsList.map(eachTab => (
                  <TabsList
                    tabDetails={eachTab}
                    key={eachTab.tabId}
                    onClickTabBtn={this.onClickTabItem}
